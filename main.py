...
def kernal(x: npt.ArrayLike, xprime: npt.ArrayLike, sigma: float) -> float:
    return math.exp( -1 * (np.linalg.norm(x - xprime, ord=2) ** 2 / 2 * sigma))
# Sln to problem 6
...
def norm(vec: npt.ArrayLike) -> float:
...
if __name__ == '__main__':
    ...
    variance = sum(norm((mean2 - i)) ** 2 for i in face_list_flat) * 1/len(face_list_flat)

    print("variance " + str(variance))
    gram = construct_gram(face_list_flat, kernal, 0.0000001)
    # Inverts gram
    ...
    x = classify(test_as_arr.flatten(), face_list_flat, Z, 0.0000001)
